		...
		<>
			...
			{mobileMenuOpen && (
				<nav
					...
					className="md:hidden absolute top-full left-0 right-0 border-t bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 z-40 shadow-lg"
				...
				>
					<div className="w-full px-4 md:px-6 py-6 space-y-4">
						...
						{navItems.map((item, index) => {
							...
							<a
								key={item.href}
							ref={index === 0 ? firstMobileItemRef : null}
							href={item.href}
							className={`
						...
						{/* Séparateur */}
						<div className="border-t my-4" role="separator" aria-hidden="true"></div>
						...
						<Button
							...
							aria-label={isLoggedIn ? "Accéder au profil utilisateur" : "Se connecter à votre compte"}
						>
							{isLoggedIn ? (
								<>
									<User className="w-4 h-4" aria-hidden="true" />
